...
def _try_connect(port_override: Optional[str] = None):
    host = _env("DB_HOST")
    port = port_override or _env("DB_PORT", "5432")
    user = _env("DB_USER")
    pwd  = _env("DB_PASSWORD")
    db   = _env("DB_NAME")
    ssl  = _env("DB_SSLMODE", "require")
    if not all([host, port, user, pwd, db]):
        raise RuntimeError("Faltam variáveis (DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME).")
...
def ensure_ping() -> Tuple[bool, str]:
    try:
        _ = qone("select 1 as ok;")
        return True, "ok"
...
def fifo_allocate(product_id: int, required_qty: float) -> List[Dict[str, Any]]:
    rows = qall("""
      ...
      select pi.id as lot_id, pi.qty as lot_qty, pi.unit_price,
    ...
    for r in rows:
        ...
        alloc.append({"lot_id": int(r["lot_id"]), "qty": take, "unit_cost": float(r.get("unit_price") or 0.0)})
        remaining -= take
    return alloc
...
def page_dashboard():
    ...
    stock = qall("select coalesce(sum(stock_qty * avg_cost),0) as val, coalesce(sum(stock_qty),0) as qty from resto.product;")[0]
    with col1:
        card_start(); st.markdown("**Valor do Estoque (CMP)**\n\n<h3 class='kpi'>{}</h3>".format(money(stock['val'])), unsafe_allow_html=True)
        st.caption("Quantidade total em estoque: {:.2f}".format(stock['qty'])); card_end()
    with col2:
        card_start(); st.markdown("**Últimos CMVs (mensal)**"); st.caption("Crie a view v_cmv para preencher.") ; card_end()
    with col3:
        card_start(); st.markdown("**Vencimentos em 30 dias**"); st.caption("Use a aba Estoque → Lotes & Validade.") ; card_end()
...
def page_cadastros():
    ...
    with tabs[0]:
        ...
        st.dataframe(pd.DataFrame(qall("select id, name, abbr, base_hint from resto.unit order by abbr;")), use_container_width=True, hide_index=True)
        card_end()
    ...
    with tabs[1]:
        ...
        st.dataframe(pd.DataFrame(qall("select id, name from resto.category order by name;")), use_container_width=True, hide_index=True)
        card_end()
    ...
    with tabs[2]:
        ...
        if ok and name:
            qexec("insert into resto.supplier(name, cnpj, ie, email, phone) values (%s,%s,%s,%s,%s);", (name, cnpj, ie, email, phone))
            st.success("Fornecedor salvo!")
        st.dataframe(pd.DataFrame(qall("select id, name, cnpj, ie, email, phone from resto.supplier order by name;")), use_container_width=True, hide_index=True)
        card_end()
    ...
    with tabs[3]:
        ...
        with st.form("form_prod"):
            ...
            name = st.text_input("Nome *")
            category = st.selectbox("Categoria", options=[(c['id'], c['name']) for c in cats] if cats else [], format_func=lambda x: x[1] if isinstance(x, tuple) else x, index=0 if cats else None)
            unit = st.selectbox("Unidade *", options=[(u['id'], u['abbr']) for u in units] if units else [], format_func=lambda x: x[1] if isinstance(x, tuple) else x, index=0 if units else None)
        ...
        if ok and name and unit:
            ...
            """, (code, name, category[0] if category else None, unit[0] if unit else None, ncm, cest, cfop, csosn, cst_icms, ali_icms, cst_pis, ali_pis, cst_cof, ali_cof, iss, is_sale, is_ing, markup))
            st.success("Produto salvo!")
        st.dataframe(pd.DataFrame(qall("select id, code, name, stock_qty, avg_cost, last_cost from resto.product order by name;")), use_container_width=True, hide_index=True)
        card_end()
...
# ===================== IMPORTADOR DE EXTRATO (C6/OFX/CSV) =====================
...
def _classify_tx(desc: str, amount: float) -> Dict[str, Any]:
    ...
    if kind == 'IN':
        cat = qone("select id, name from resto.cash_category where name='Outros Recebimentos';")
    else:
        cat = qone("select id, name from resto.cash_category where name='Outros Pagamentos';")
...
def page_diagnostico(err_msg: str):
    header("🩺 Diagnóstico de conexão", "App carregou, mas a conexão com o banco falhou.")
    card_start()
    st.markdown("### Resultado do teste")
    st.error(err_msg)
    st.markdown("### Variáveis detectadas (secrets/env)")
    st.code({
    ...
    st.markdown("### Tentar outra porta automaticamente (Supabase costuma aceitar 5432 **ou** 6543)")
    col1, col2 = st.columns(2)
    with col1:
        if st.button("Testar porta 5432"):
            try:
                with _try_connect("5432") as _:
                    st.success("Conectou com 5432 ✅")
            except Exception as e:
                st.error(f"Falha na 5432: {e}")
    with col2:
        if st.button("Testar porta 6543 (pooler/pgbouncer)"):
            try:
                with _try_connect("6543") as _:
                    st.success("Conectou com 6543 ✅")
            except Exception as e:
                st.error(f"Falha na 6543: {e}")
    st.markdown("### Dicas rápidas")
    st.write("- Confira o **DB_USER** do Supabase: geralmente `postgres.<project-ref>`")
    st.write("- Em `DB_HOST`, use o **pooler**: `aws-1-us-east-1.pooler.supabase.com`")
    st.write("- Garanta que `DB_SSLMODE=require`")
    st.write("- Troque a porta entre 5432 e 6543 caso precise")
    card_end()
...
def main():
    ok, msg = ensure_ping()
    if not ok:
        # Mostra diagnóstico e não encerra a app
        page_diagnostico(msg)
        return
    ...
    page = st.sidebar.radio(
        "Menu",
        ["Painel", "Cadastros", "Compras", "Vendas", "Receitas & Preços", "Financeiro", "Importar Extrato"],
        index=0
    )
