...
def _load_c6_csv(raw: bytes):
    ...
    # Procura cabeçalho do C6 (case/acento tolerantes)
    norm_lines = [_norm(ln) for ln in txt.splitlines()]
    target = _norm("Data Lançamento,Data Contábil,Título,Descrição,Entrada(R$),Saída(R$),Saldo do Dia(R$)")
    start = None
    ...
    # Mapeia colunas com tolerância de acentos
    cols = { _norm(c): c for c in df.columns }
    d_lanc = cols.get(_norm("Data Lançamento")) or cols.get(_norm("Data"))
    desc   = cols.get(_norm("Descrição")) or cols.get(_norm("Titulo")) or cols.get(_norm("Título"))
    ent    = cols.get(_norm("Entrada(R$)"))
    sai    = cols.get(_norm("Saída(R$)"))
...
def _load_csv_generic(raw: bytes):
    ...
    # datas
    date_key = None
    for k in ("data lançamento","data lancamento","data","date","data contabil","data contábil","entry_date"):
        if k in nmap: date_key = nmap[k]; break
    # descrições
    desc_key = None
    for k in ("descrição","descricao","description","histórico","historico","titulo","título","detalhe","detalhes"):
        if k in nmap: desc_key = nmap[k]; break
    # valores (uma coluna) ou crédito/débito (duas colunas)
    val_key = None
    for k in ("valor","amount","valor lançamento","valor lancamento"):
        if k in nmap: val_key = nmap[k]; break
    cred_key = None
    for k in ("entrada(r$)","credito","crédito","credit"):
        if k in nmap: cred_key = nmap[k]; break
    deb_key = None
    for k in ("saída(r$)","saida(r$)","debito","débito","debit"):
