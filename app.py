...
def page_cadastros():
    ...
    with tabs[3]:
        ...
        st.caption(
            "Use a planilha exportada do OLACLICK. "
            "Serão usados: Categoria do produto, Nome do produto, Descrição (ainda não salva) "
            "e o campo **Preço** como valor de venda."
        )
        ...
        if file_prod is not None:
            ...
            if df_imp is not None:
                ...
                # ➜ AGORA o preço principal é sempre PREÇO (valor de venda)
                col_preco  = norm_map.get("preco") or norm_map.get("preço")
                # usado só como fallback se o preço estiver vazio/zero
                col_preco_desc = norm_map.get("preco com desconto") or norm_map.get("preço com desconto")
                ...
                if not col_nome or not col_preco:
                    ...
                    st.write("Pré-visualização dos dados lidos da planilha:")
                    preview_cols = [
                        c for c in [col_cat, col_nome, col_desc, col_preco, col_preco_desc] if c
                    ]
                    st.dataframe(df_imp[preview_cols].head(30), use_container_width=True)
                    ...
                    if st.button("🚀 Importar produtos da planilha", key="btn_import_prod_olaclick"):
                        ...
                        for _, row in df_imp.iterrows():
                            ...
                            # categoria (texto exatamente como está na planilha)
                            if col_cat and not pd.isna(row[col_cat]):
                                category = str(row[col_cat]).strip() or None
                            else:
                                category = None

                            # preço de venda: sempre usa PREÇO.
                            price_val = 0.0
                            if not pd.isna(row[col_preco]):
                                price_val = _to_float(row[col_preco])

                            # se preço vier 0 e existir "Preço com desconto", usa como fallback
                            if (price_val == 0.0) and col_preco_desc and not pd.isna(row.get(col_preco_desc)):
                                price_val = _to_float(row[col_preco_desc])
                            ...
                            try:
                                ...
                                """, (
                                    ...
                                    None,                    # barcode
                                    0.0,                     # min_stock
                                    0.0,                     # last_cost (custo você vai alimentar depois)
                                    True,                    # active
                                    float(price_val or 0.0), # sale_price (PREÇO)
                                    True,                    # is_sale_item
                                    False,                   # is_ingredient
                                    0.0,                     # default_markup
...
def page_financeiro():
    ...
    # ---------- Aba: 📆 Comparativo ----------
                # === Helpers para gerar PDF do Comparativo ===
                ...
                def _ensure_reportlab_runtime_cmp() -> bool:
                    """
                    Garante que o pacote reportlab está instalado.
                    Se não estiver, só avisa (não quebra o app).
                    """
                    try:
                ...
                def _build_pdf_bytes_cmp(titulo: str, df_show: pd.DataFrame,
                                         total_e: float, total_s: float, total_res: float) -> bytes:
                    """
                    ...
                    # Totais
                    dados_totais = [
                        ["Entradas", "Saídas", "Resultado (E − S)"],
                        [_brl_cmp(total_e), _brl_cmp(total_s), _brl_cmp(total_res)],
                    ]
                    tbl_tot = Table(dados_totais, colWidths=[200, 200, 200])
                    ...
                    if df_show is not None and not df_show.empty:
                        col_periodo = df_show.columns[0]   # "periodo" ou "ano"
                        dados = [[str(col_periodo).capitalize(), "Entradas", "Saídas", "Saldo"]]
                        for _, linha in df_show.iterrows():
                            dados.append([
                                str(linha[col_periodo]),
                                _brl_cmp(linha["entradas"]),
                                _brl_cmp(linha["saidas"]),
                                _brl_cmp(linha["saldo"]),
                            ])
            
                        tbl = Table(dados, colWidths=[160, 120, 120, 120])
        ...
        if modo in ("Mensal (12m)", "Semestral (6m)"):
            ...
            if not dfc.empty:
                ...
                if has_pdf:
                    ...
                    titulo = f"Comparativo Financeiro – {tipo} ({ini} a {fim})"
                ...
                with bc1:
                    st.download_button("⬇️ Exportar CSV (Comparativo Mensal/Semestral)",
                                       data=csv,
                                       file_name="comparativo_mensal.csv",
                                       mime="text/csv")
                with bc2:
                    if has_pdf and pdf_bytes:
                        st.download_button("📄 Exportar PDF (Comparativo)",
                                           data=pdf_bytes,
        ...
        # ----------------- ANUAL (5 anos) -----------------
            ...
            if not dfy.empty:
                ...
                if has_pdf:
                    ...
                    titulo = f"Comparativo Financeiro – Anual ({ano_ini} a {ano_fim})"
                ...
                bc1, bc2 = st.columns(2)
                with bc1:
                    st.download_button("⬇️ Exportar CSV (Comparativo Anual)",
                                       data=csv,
                                       file_name="comparativo_anual.csv",
                                       mime="text/csv")
                with bc2:
                    if has_pdf and pdf_bytes:
                        st.download_button("📄 Exportar PDF (Comparativo Anual)",
                                           data=pdf_bytes,
